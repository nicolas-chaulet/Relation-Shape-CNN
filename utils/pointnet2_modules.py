...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(
        ...
        if self.npoint is not None:
            ...
            new_xyz = (
                tp.gather_operation(xyz_flipped, fps_idx).transpose(1, 2).contiguous()
            )
        ...
        for i in range(len(self.groupers)):
            new_features = (
                self.groupers[i](xyz, new_xyz, features, fps_idx)
                if self.npoint is not None
                else self.groupers[i](xyz, new_xyz, features)
            )  # (B, C, npoint, nsample)
        ...
        *,
        ...
        if first_layer:
            ...
            mapping_func2 = nn.Conv2d(
                in_channels=math.floor(C_out / 2),
                out_channels=16,
                kernel_size=(1, 1),
                stride=(1, 1),
                bias=bias,
            ...
            mapping_func2 = nn.Conv2d(
                in_channels=math.floor(C_out / 4),
                out_channels=C_in,
                kernel_size=(1, 1),
                stride=(1, 1),
                bias=bias,
        ...
        if npoint is not None:
            ...
            cr_mapping = nn.Conv1d(
                in_channels=C_in if not first_layer else 16,
                out_channels=C_out,
                kernel_size=1,
                stride=1,
                bias=bias,
        ...
        for i in range(len(radii)):
            ...
            self.groupers.append(
                pn2_utils.QueryAndGroup(radius, nsample, use_xyz=use_xyz)
                if npoint is not None
                else pn2_utils.GroupAll(use_xyz)
            )
            mlp_spec = mlps[i]
            if use_xyz:
                mlp_spec[0] += 3
