...
parser = argparse.ArgumentParser(
    description="Relation-Shape CNN Shape Classification Training"
)
...
def main():
    ...
    train_dataset = ModelNet40Cls(
        num_points=args.num_points, root=args.data_root, transforms=train_transforms
    )
    ...
    lr_lbmd = lambda e: max(
        args.lr_decay ** (e // args.decay_step), args.lr_clip / args.base_lr
    )
    bnm_lmbd = lambda e: max(
        args.bn_momentum * args.bn_decay ** (e // args.decay_step), args.bnm_clip
    )
...
def train(
    ...
    for epoch in range(args.epochs):
        for i, data in enumerate(train_dataloader, 0):
            ...
            fps_idx = tp.furthest_point_sample(points, 1200)  # (B, npoint)
            fps_idx = fps_idx[:, np.random.choice(1200, args.num_points, False)]
            points = (
                tp.gather_operation(points.transpose(1, 2).contiguous(), fps_idx)
                .transpose(1, 2)
                .contiguous()
            )  # (B, N, 3)
            ...
            if (
                args.evaluate
                and batch_count % int(args.val_freq_epoch * num_batch) == 0
            ):
...
def validate(test_dataloader, model, criterion, args, iter):
    ...
    for j, data in enumerate(test_dataloader, 0):
        ...
        points, target = points.cuda(), target.cuda()
        points, target = (
            Variable(points, volatile=True),
            Variable(target, volatile=True),
        )

        # fastest point sampling
        fps_idx = tp.furthest_point_sample(points, args.num_points)  # (B, npoint)
        # fps_idx = fps_idx[:, np.random.choice(1200, args.num_points, False)]
        points = (
            tp.gather_operation(points.transpose(1, 2).contiguous(), fps_idx)
            .transpose(1, 2)
    ...
    if acc > g_acc:
        ...
        torch.save(
            model.state_dict(),
            "%s/cls_ssn_iter_%d_acc_%0.6f.pth" % (args.save_path, iter, acc),
        )
    model.train()
