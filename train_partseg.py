...
parser = argparse.ArgumentParser(
    description="Relation-Shape CNN Shape Part Segmentation Training"
)
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm.tqdm(train_dataloader) as loader:
            for i, data in enumerate(loader, 0):
                ...
                loader.set_postfix(
                    train_loss=loss.data.clone(), lr=lr_scheduler.get_lr()[0]
                )
                batch_count += 1
...
def validate(test_dataloader, model, criterion, args, iter):
    ...
    for _, data in enumerate(test_dataloader, 0):
        ...
        with torch.no_grad():
            pred = model(points, batch_one_hot_cls)
            predl = pred.view(-1, args.num_classes)
            targetl = target.view(-1, 1)[:, 0]
            loss = criterion(predl, targetl)
        ...
        for b in range(len(cls)):
            ...
            for l in seg_classes[cat]:
                if torch.sum((segl == l) | (segp == l)) == 0:
                    ...
                    part_ious[l - seg_classes[cat][0]] = float(
                        torch.sum((segl == l) & (segp == l))
                    ) / float(torch.sum((segl == l) | (segp == l)))
